...
This is just a simple example for a working Discord bot. By default it will react on `!price` (you may change it) command and output embedded message with `appid`, `market_hash_name` and its `price`. You can change appearance and some styling directly from a config file.
